...
UINT CManagerMarket::HandleAllRecvPacketThread()
{
	while(m_bRunning)
	{
		Market * pOnePacket = m_cRecvPacketList.pop();
		if(NULL == pOnePacket)
		{
			Sleep(10);
			continue;
...
BOOL CManagerMarket::ConnetToTS(const char* strIP, int nPort)
{
	return CNetManagerMakket::GetInstance()->ConnectToTS(strIP,nPort);
}
...
{
	m_cRecvPacketList.push(pOnePacket);
}
...
BOOL CManagerMarket::ParserJsonResponse(string& strJson, DWORD dwCmd)
{
	...
	case GWTS_CMD_LOGIN:	//登录
 	...
 	{
		string Str = ::UTF8ToGB(tRspHead.sMsg.c_str());
 		this->PostTSMessage(dwCmd,NULL,(LPARAM)Str.c_str());
 	}
...
BOOL CManagerMarket::ParserHead(tagRspMarketHead& tRspHead, Json::Value& jsAns)
{
	...
	pRspHead->iFrom = GetIntFromJsonValue(jsAns,"from");
	pRspHead->iSeq = GetIntFromJsonValue(jsAns,"seq");
	pRspHead->bSuccess = GetIntFromJsonValue(jsAns,"success");
	pRspHead->iCmd = GetIntFromJsonValue(jsAns,"cmd");
	pRspHead->iRet = GetIntFromJsonValue(jsAns,"ret");
	pRspHead->sMsg = GetStringFromJsonValue(jsAns,"msg");
// 	GET_JSON_INTVALUE(pRspHead, iFrom, jsAns);
// 	GET_JSON_STRINGVALUE(pRspHead, , jsAns);
